...
export class FileParser {
  ...
  parse(): FileData {
    ...
    stateMachine.addStateAction(ParsingState.Record, (line: string) => {
  ...
  private parseMetadataProject(line: string): CompoundName {
    ...
    if (
      this.activeProjects.find(searchForCompoundProject(parsedProject)) ||
  ...
  private parseProject(line: string): EstimatedProject {
    ...
    if (
      splitted.length !== 2 ||
      this.currentRecord === undefined ||
      !name.match(PROJECT_NAME_REGEX)
    ) {
      throw new Error("invalid record");
    }
    const compoundName = new CompoundName(name);
    if (
      this.currentRecord.projects
    ...
    if (
      !this.activeProjects.find(searchForCompoundProject(compoundName)) &&
  ...
  private parseMark(value: string): Mark {
    ...
    return trimmedValue === "-" ? undefined : (parseInt(trimmedValue) as Mark);
  }
}
