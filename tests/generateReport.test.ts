  ...
  records: [
    {
      ...
      projects: [{ name: firstProject, mark: 4 }],
    },
    {
    ...
    },
    {
      ...
      projects: [{ name: secondProject, mark: 5 }],
    },
  ],
};
...
describe("generateReport", () => {
  test("absolute report", () => {
    expect(
      generateReport(DATA_SAMPLE, options("2022-07-01", "2022-07-04", true))
    ).toMatchSnapshot();
  });

  test("absolute report for subset days", () => {
    expect(
      generateReport(DATA_SAMPLE, options("2022-07-02", "2022-07-03", true))
    ).toMatchSnapshot();
  });

  test("relative report", () => {
    expect(
      generateReport(DATA_SAMPLE, options("2022-07-01", "2022-07-04", false))
    ).toMatchSnapshot();
  });

  test("relative report", () => {
    expect(
      generateReport(DATA_SAMPLE, options("2022-07-02", "2022-07-03", false))
    ).toMatchSnapshot();
  });
  ...
  test("empty day report", () => {
    expect(
      generateReport(
        { metadata: DATA_SAMPLE.metadata, records: [] },
        options("2022-07-01", "2022-07-04", false)
      )
    ).toMatchSnapshot();
  });
});
